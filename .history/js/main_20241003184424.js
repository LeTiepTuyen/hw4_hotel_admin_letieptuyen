...
function renderAdminRooms() {
  ...
  rooms.forEach((room) => {
    ...
    <tr class="hover:bg-gray-50 transition duration-300 ease-in-out">
        <td class="px-6 py-4 whitespace-no-wrap text-sm font-medium text-gray-900">${room.id}</td>
        <td class="px-6 py-4 whitespace-no-wrap text-sm text-gray-900">${room.name}</td>
        <td class="px-6 py-4 whitespace-no-wrap text-sm text-center">
          <div class="w-16 h-16 overflow-hidden flex items-center justify-center mx-auto">
            <img src="../${room.imageUrl}" alt="${room.name}" class="w-full h-full object-contain">
          </div>
        </td>
        <td class="px-6 py-4 whitespace-no-wrap text-sm text-gray-900">${room.price}</td>
        <td class="px-6 py-4 whitespace-no-wrap flex justify-center items-center space-x-4">
          <button class="p-2 bg-blue-500 hover:bg-blue-600 text-white rounded-full shadow-md transition duration-200 ease-in-out" onclick="showUpdateForm('${room.id}')">
            <img src="../assets/images/update-icon.png" alt="Update" class="w-5 h-5">
          </button>
          <button class="p-2 bg-red-500 hover:bg-red-600 text-white rounded-full shadow-md transition duration-200 ease-in-out" onclick="deleteRoom('${room.id}'); renderAdminRooms(); renderHomepageRooms();">
            <img src="../assets/images/delete-icon.png" alt="Delete" class="w-5 h-5">
          </button>
        </td>
      </tr>
`;
