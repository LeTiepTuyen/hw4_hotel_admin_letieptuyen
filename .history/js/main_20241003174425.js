  ...
  rooms.forEach((room) => {
    ...
    <tr>
        ...
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500 flex justify-between">
            <button class="bg-red-500 text-white text-xs px-3 py-1 rounded-lg m-2" onclick="showUpdateForm('${room.id}')">Update</button>
            <button class="bg-red-500 text-white text-xs px-3 py-1 rounded-lg m-2" onclick="deleteRoom('${room.id}'); renderAdminRooms(); renderHomepageRooms();">Delete</button>
        </td>
    </tr>
`;
