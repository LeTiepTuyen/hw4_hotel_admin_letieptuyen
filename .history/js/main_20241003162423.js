    ...
    <tr>
        ...
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500">
            <div class="w-16 h-16 overflow-hidden flex items-center justify-center">
                <img src="${room.imageUrl}" alt="${room.name}" class="object-contain max-w-full max-h-full">
            </div>
        </td>
        ...
        <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-500 flex justify-between">
            <button class="bg-yellow-500 text-white text-xs px-3 py-1 rounded-lg mr-2" onclick="showUpdateForm('${room.id}')">Update</button>
            <button class="bg-red-500 text-white text-xs px-3 py-1 rounded-lg" onclick="deleteRoom('${room.id}'); renderAdminRooms(); renderHomepageRooms();">Delete</button>
        </td>
    </tr>
`;
    ...
    rooms.forEach((room) => {
      const roomCard = `
        <div class="bg-white p-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
          <img src="${room.imageUrl}" alt="${room.name}" class="w-full h-48 object-cover rounded-t-lg">
          <div class="mt-4">
